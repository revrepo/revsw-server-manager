...
class Nagios():
    ...
    def _api_call(self, relative_url, method="GET", data=None):
        ...
        if not response_data['success']:
            logger.error(
                "nagios response not set with success flag:\n%s" %
                pformat(response_data))
            raise Exception("response success is not set.")

        if response.status_code != 200:
            logger.error(
                "nagios response with status code %d:\n%s" % (
                    response.status_code, pformat(response_data)
                )
            )
            raise Exception(
                "nagios response status-code %d" % response.status_code
            )
    ...
    def schedule_downtime(self, server, duration=1200):
        logger.info(
            "declaring scheduled downtime on %s for %d seconds" % (
                server, duration
            )
        )
        data = {
...
#         logger.debug(
#             "Looking for the nsone monitoring job of host %s" % host
#         )
